...
*/
	...
	*/
	...
	/* scarface addition */
	function &GetAllAssoc($sql, $inputarr=false)
	{
	    $t = $this->fetchMode;
    ...
    /* scarface addition */
	function &GetAllAssocLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$secs2cache=0)
	{
	    $t = $this->fetchMode;
	    $this->SetFetchMode(ADODB_FETCH_ASSOC);
...
*/
    ...
    /* scarface addition */
	function &GetItem($sql,$inputarr=false)
	{
	    $t = $this->fetchMode;
	    $this->SetFetchMode(ADODB_FETCH_ASSOC);
		$arr =& $this->GetRow($sql,$inputarr);
		$this->SetFetchMode($t);
		return $arr;
	}
    ...
    function insertRecord($table, $record){
        ...
        $query = "INSERT INTO $table (".$fields.") VALUES (".$wildcards.")";
    
        $result =& $this->Execute($query,$bindvars);
    ...
    /* scarface addition */
    function updateRecord($table, $record, $key){
        $fieldvals = "";
        $bindvars = array();
        $keyVal = false;
        foreach ($record as $k => $v){
        ...
        if (count($bindvars) == 0) return true;
        
        if ($keyVal === false){
            throw new Exception('Key index not not found in record');
        }

        $bindvars[] = $keyVal;
        $query = "UPDATE ".$table." SET ".$fieldvals." WHERE ".$key." = ?";
    ...
    /* scarface addition */
    function deleteRecord($table, $conds){
