...
class ClassMethodNotExistException extends Exception{}
    ...
    **/
    private static function once($dp)
    {
        ...
        if(!isset(self::$has[$dp]))
        {
            ...
            $file = self::root() . '/' . str_replace('.','/',$dp).'.php';

            if (!is_file($file)) throw new FileNotFoundException($file." does not exist");
